...
public class printMatrixPath {
    ...
    public printMatrixPath(int[][] matrix, int matrixLenghRow, int matrixLenghColumn) {
        this.matrix = matrix;
    ...
    public static void main(String[] args) {
        ...
        printMatrixPath shortestPath = new printMatrixPath(matrix1, mRow, mCol);
    ...
    private int find(int row) {
        ...
        for(int currRow=row-1; currRow <= row+1; currRow++) {
            next[0] = getBoundaryRow( currRow );
            res += next[0] + " ";
            sum = matrix[currRow][0];
